...
public class Client2 {
    ...
    // DNS коды сообщений
    private static final byte DNS_DISCOVER = 30;   // Обнаружение DNS сервера
    private static final byte DNS_ANNOUNCE = 31;   // Объявление DNS сервера
    private static final byte DNS_REGISTER = 32;   // Регистрация домена
    private static final byte DNS_RESOLVE = 33;    // Запрос разрешения имени
    private static final byte DNS_RESPONSE = 34;   // Ответ на DNS запрос
        ...
        private void handleDnsResponse(String serverMAC, String serverIP, String data) {
            ...
            if (data.startsWith("DNS RESOLVE OK")) {
                ...
                if (parts.length >= 5) { // Должно быть не менее 5 частей
                    ...
                    dnsCache.put(domain, ip); // Сохраняем домен - IP (ключ - значение)
                }
            }
        }
        /**
